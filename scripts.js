...
function createTable(team) {
  ...
  players.filter(player => player.team === team).forEach(player => {
    ...
    row.innerHTML = `<td>${player.name}</td>${Array.from({ length: 10 }, (_, i) => `<td><input type="text" id="${player.name}-score${i}" value="${player.scores[i] || ''}" onblur="updateScores()" /></td>`).join('')}<td>${player.bookAverage}</td><td>${player.average}</td><td>${player.handicap}</td>`;
  });
